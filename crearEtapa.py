...
# logicamente a lo sumo le vas a entrar con 2 ceros y 2 polos
    ...
    if(len(polos) == 1):    # si solo se mando un polo
        ...
        den = [1/(polos[0].real*polos[1].real), 1/abs(polos[0].real) + 1/abs(polos[1].real), 1] # y matematicamente los coeficientes serian esos
    elif(len(polos) == 0):
        den = 1
    ...
    if(len(ceros) == 1):
        ...
        elif(ceros[0].real == 0):                    #si entra aca es un cero en origen del plano S, sus coeficientes van a ser esos
            num = [0, 1, 0]
        else:
            num = [0, 1/abs(ceros[0].real), 0]
    elif(len(ceros) == 2):
        if(ceros[0].Q == 0 and ceros[1].Q == 0):                # Q = 0 es nuestra manera se saber que la singularidad esta sobre origen del plano S
            ...
            num = [1/(ceros[0].real*ceros[1].real), 1/abs(ceros[0].real) + 1/abs(ceros[1].real), 1]
    elif(len(ceros) == 0):
        num = 1
